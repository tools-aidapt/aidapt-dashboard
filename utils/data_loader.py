...
Sheet structure per tab (KPI Mart v4):
  CLIENTS, KPI_DAILY:
    Row 1 = headers, Row 2 = instructions, Row 3+ = data

  OPPORTUNITIES, OPP_FINANCIALS, KPI_MONTHLY, SOLUTIONS,
  TICKET_SENTIMENT, BASELINES:
    Row 1 = banner title, Row 2 = headers, Row 3 = instructions, Row 4+ = data
"""
...
# Exact row config per tab — no auto-detection needed
# (header_idx, data_start) — 0-indexed into rows list
TAB_CONFIG = {
    "CLIENTS":          (0, 2),   # row1=headers, row2=instructions, row3+=data
    "KPI_DAILY":        (0, 2),   # row1=headers, row2=instructions, row3+=data
    "OPPORTUNITIES":    (1, 3),   # row1=banner, row2=headers, row3=instructions, row4+=data
    "OPP_FINANCIALS":   (1, 3),
    "KPI_MONTHLY":      (1, 3),
    "SOLUTIONS":        (1, 3),
    "TICKET_SENTIMENT": (1, 3),
    "BASELINES":        (1, 3),
}
...
def _read_tab(sheets, name):
    ...
    # Use FORMATTED_VALUE so Google Sheets formulas return computed values,
    # not the raw formula strings like =SUMPRODUCT(...)
    try:
        rows = ws.get_all_values(value_render_option="FORMATTED_VALUE")
    except TypeError:
        # Older gspread versions don't support value_render_option keyword
        rows = ws.get(value_render_option="FORMATTED_VALUE")
    except Exception:
    ...
    header_idx, data_start = TAB_CONFIG.get(name, (0, 2))
    ...
    # Drop placeholder / separator rows
    first_col = df.iloc[:, 0].astype(str).str.strip()
    df = df[first_col.ne("")]
    df = df[~first_col.str.startswith("▼")]
    df = df[~first_col.str.startswith("C00X")]
    df = df[~first_col.str.contains("ADD NEW", na=False)]
    df = df[~first_col.str.match(r"^C0+X", na=False)]
...
def _clean_df(df):
    ...
    date_keywords = ['date', 'month', 'week', 'start', 'end', 'at', 'go_live']
    for col in df.columns:
...
def _load_demo_data():
    ...
    clients = pd.DataFrame([
        {"client_id": "C002", "client_code": "kenafric", "client_name": "Kenafric Industries",
         ...
         "region": "East Africa", "avg_minutes_per_run": 8},
        {"client_id": "C003", "client_code": "tilecentre", "client_name": "TCC Group",
         ...
         "region": "East Africa", "avg_minutes_per_run": 22},
        {"client_id": "C001", "client_code": "bipl", "client_name": "Bank Islami",
         ...
         "region": "Pakistan", "avg_minutes_per_run": 20},
    ])
    clients["go_live_date"] = pd.to_datetime(clients["go_live_date"])

    # ── OPPORTUNITIES ─────────────────────────────────────────────────────────
    opportunities = pd.DataFrame([
        {"client_id": "C002", "opp_id": "OPP-001", "opp_name": "Invoice Processing Automation",
        ...
        {"client_id": "C002", "opp_id": "OPP-002", "opp_name": "Demand Forecasting",
        ...
        {"client_id": "C002", "opp_id": "OPP-003", "opp_name": "HR Onboarding Automation",
        ...
        {"client_id": "C003", "opp_id": "OPP-004", "opp_name": "Network Fault Prediction",
        ...
        {"client_id": "C003", "opp_id": "OPP-005", "opp_name": "Customer Churn Prediction",
        ...
        {"client_id": "C003", "opp_id": "OPP-006", "opp_name": "Call Centre AI Assist",
        ...
        {"client_id": "C001", "opp_id": "OPP-007", "opp_name": "AML Transaction Monitoring",
         "function": "Compliance", "ai_pattern": "Predictive ML",
         "value_type_primary": "Quality", "value_type_secondary": "OpEx",
         "priority_score": 9.4, "feasibility_score": 5, "value_score": 5,
         "initiative_status": "Live", "buy_build": "Build", "hitl_level": "Partial"},
        {"client_id": "C001", "opp_id": "OPP-008", "opp_name": "Loan Application Processing",
         "function": "Retail Banking", "ai_pattern": "Document AI",
         "value_type_primary": "Productivity", "value_type_secondary": "Quality",
         "priority_score": 8.5, "feasibility_score": 4, "value_score": 5,
         "initiative_status": "Live", "buy_build": "Buy", "hitl_level": "Partial"},
        {"client_id": "C001", "opp_id": "OPP-009", "opp_name": "Regulatory Reporting Auto",
    ...
    opp_financials = pd.DataFrame([
        {"client_id":"C002","opp_id":"OPP-001","minutes_saved_per_unit":8,
        ...
        {"client_id":"C002","opp_id":"OPP-002","minutes_saved_per_unit":15,
        ...
        {"client_id":"C002","opp_id":"OPP-003","minutes_saved_per_unit":12,
        ...
        {"client_id":"C003","opp_id":"OPP-004","minutes_saved_per_unit":20,
        ...
        {"client_id":"C003","opp_id":"OPP-005","minutes_saved_per_unit":30,
        ...
        {"client_id":"C003","opp_id":"OPP-006","minutes_saved_per_unit":10,
        ...
        {"client_id":"C001","opp_id":"OPP-007","minutes_saved_per_unit":25,
        ...
        {"client_id":"C001","opp_id":"OPP-008","minutes_saved_per_unit":18,
        ...
        {"client_id":"C001","opp_id":"OPP-009","minutes_saved_per_unit":12,
    ...
    # ── KPI_DAILY ─────────────────────────────────────────────────────────────
    mins_map = {"C002": 8, "C003": 22, "C001": 20}
    rows = []

    # C002 Kenafric: 64 days from 2026-01-06
    for d in range(64):
    ...
    # C003 TCC: 38 days from 2026-02-01
    for d in range(38):
        ...
        hp  = max(0, 4 - d // 10)
        rows.append({"client_id":"C003","date":dt,"solutions_deployed":2,
                     "automation_runs_success":ro,"automation_runs_failed":rf,
                     "support_tickets_created":tc,
    ...
    # C001 Bank Islami: 87 days from 2025-12-15
    for d in range(87):
        ...
        hp  = max(0, 5 - d // 15)
        rows.append({"client_id":"C001","date":dt,"solutions_deployed":3,
                     "automation_runs_success":ro,"automation_runs_failed":rf,
                     "support_tickets_created":tc,
    ...
    # ── KPI_MONTHLY ───────────────────────────────────────────────────────────
    monthly_raw = [
        {"client_id":"C002","month":"2026-01","cost_savings_usd":18500,"delivery_cost_usd":8200,"planned_roi_multiple":2.5},
        {"client_id":"C002","month":"2026-02","cost_savings_usd":24800,"delivery_cost_usd":8200,"planned_roi_multiple":2.5},
        {"client_id":"C002","month":"2026-03","cost_savings_usd":28200,"delivery_cost_usd":8200,"planned_roi_multiple":2.5},
        {"client_id":"C003","month":"2026-02","cost_savings_usd":32000,"delivery_cost_usd":11500,"planned_roi_multiple":3.2},
        {"client_id":"C003","month":"2026-03","cost_savings_usd":41500,"delivery_cost_usd":11500,"planned_roi_multiple":3.2},
        {"client_id":"C001","month":"2025-12","cost_savings_usd":42000,"delivery_cost_usd":18000,"planned_roi_multiple":4.1},
        {"client_id":"C001","month":"2026-01","cost_savings_usd":55000,"delivery_cost_usd":18000,"planned_roi_multiple":4.1},
        {"client_id":"C001","month":"2026-02","cost_savings_usd":68000,"delivery_cost_usd":18000,"planned_roi_multiple":4.1},
        {"client_id":"C001","month":"2026-03","cost_savings_usd":71000,"delivery_cost_usd":18000,"planned_roi_multiple":4.1},
    ]
    ...
    solutions = pd.DataFrame([
        {"client_id":"C002","opp_id":"OPP-001","solution_name":"Invoice Intelligence Platform",
         "solution_type":"Document AI","go_live_date":"2026-01-06","phase":"Live",
         "fte_impacted":85,"version":"v1.2","notes":"Processing 400+ invoices/day"},
        {"client_id":"C002","opp_id":"OPP-002","solution_name":"Demand Signal Engine",
         "solution_type":"Predictive ML","go_live_date":"2026-02-15","phase":"Pilot",
         "fte_impacted":40,"version":"v0.8","notes":"3-month pilot, go-live Q2"},
        {"client_id":"C003","opp_id":"OPP-004","solution_name":"NetGuard Fault Predictor",
         "solution_type":"Predictive ML","go_live_date":"2026-02-01","phase":"Live",
         "fte_impacted":120,"version":"v1.0","notes":"Covering all 12 network zones"},
        {"client_id":"C003","opp_id":"OPP-005","solution_name":"ChurnShield Analytics",
         "solution_type":"Predictive ML","go_live_date":"2026-02-01","phase":"Live",
         "fte_impacted":65,"version":"v1.1","notes":"Integrated with CRM"},
        {"client_id":"C001","opp_id":"OPP-007","solution_name":"AML Sentinel",
         "solution_type":"Predictive ML","go_live_date":"2025-12-15","phase":"Live",
         "fte_impacted":200,"version":"v2.0","notes":"Regulatory approved"},
        {"client_id":"C001","opp_id":"OPP-008","solution_name":"LoanFlow Automation",
         "solution_type":"Document AI","go_live_date":"2025-12-15","phase":"Live",
         "fte_impacted":150,"version":"v1.3","notes":"Handles retail + SME loans"},
        {"client_id":"C001","opp_id":"OPP-009","solution_name":"RegReport Auto",
         "solution_type":"RPA","go_live_date":"2026-01-15","phase":"Pilot",
    ...
    ts_rows = []
    configs = [
        ("C002", datetime.date(2026, 1,  5), 5, 3),
        ("C003", datetime.date(2026, 2,  2), 6, 3),
        ("C001", datetime.date(2025,12, 15), 4, 6),
    ]
    ...
    baselines = pd.DataFrame([
        {"client_id":"C002","kpi_name":"avg_resolution_hrs","baseline_value":52,"unit":"hours",
         "baseline_start":"2025-11-25","baseline_end":"2025-12-25"},
        {"client_id":"C002","kpi_name":"tickets_open","baseline_value":18,"unit":"count",
         "baseline_start":"2025-11-25","baseline_end":"2025-12-25"},
        {"client_id":"C002","kpi_name":"support_tickets_created","baseline_value":9.2,"unit":"count/day",
         "baseline_start":"2025-11-25","baseline_end":"2025-12-25"},
        {"client_id":"C002","kpi_name":"high_priority_count","baseline_value":4.5,"unit":"count/day",
         "baseline_start":"2025-11-25","baseline_end":"2025-12-25"},
        {"client_id":"C003","kpi_name":"avg_resolution_hrs","baseline_value":68,"unit":"hours",
         "baseline_start":"2026-01-02","baseline_end":"2026-01-30"},
        {"client_id":"C003","kpi_name":"tickets_open","baseline_value":22,"unit":"count",
         "baseline_start":"2026-01-02","baseline_end":"2026-01-30"},
        {"client_id":"C003","kpi_name":"support_tickets_created","baseline_value":13.8,"unit":"count/day",
         "baseline_start":"2026-01-02","baseline_end":"2026-01-30"},
        {"client_id":"C003","kpi_name":"high_priority_count","baseline_value":6.2,"unit":"count/day",
         "baseline_start":"2026-01-02","baseline_end":"2026-01-30"},
        {"client_id":"C001","kpi_name":"avg_resolution_hrs","baseline_value":74,"unit":"hours",
         "baseline_start":"2025-11-15","baseline_end":"2025-12-14"},
        {"client_id":"C001","kpi_name":"tickets_open","baseline_value":28,"unit":"count",
         "baseline_start":"2025-11-15","baseline_end":"2025-12-14"},
        {"client_id":"C001","kpi_name":"support_tickets_created","baseline_value":16.5,"unit":"count/day",
         "baseline_start":"2025-11-15","baseline_end":"2025-12-14"},
        {"client_id":"C001","kpi_name":"high_priority_count","baseline_value":7.8,"unit":"count/day",
         "baseline_start":"2025-11-15","baseline_end":"2025-12-14"},
        {"client_id":"C001","kpi_name":"automation_runs_success","baseline_value":0,"unit":"count",
         "baseline_start":"2025-11-15","baseline_end":"2025-12-14"},
        {"client_id":"C001","kpi_name":"hours_saved","baseline_value":0,"unit":"hours",
         "baseline_start":"2025-11-15","baseline_end":"2025-12-14"},
    ])
